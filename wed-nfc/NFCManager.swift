...
class NFCManager: NSObject {
    ...
    func write(text: String, handler: (() -> Void)? = nil) {
        ...
        isWriting = true
        let textPayload = NFCNDEFPayload.wellKnownTypeTextPayload(string: text, locale: .init(identifier: "ja_JP"))!
...
extension NFCManager: NFCNDEFReaderSessionDelegate {
    ...
    private func read(tag: NFCNDEFTag, session: NFCNDEFReaderSession) {
        tag.readNDEF { [unowned self] message, _ in
            ...
            session.invalidate()
            let text = message?.records.first?.wellKnownTypeTextPayload().0
            DispatchQueue.main.async {
